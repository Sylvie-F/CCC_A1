...
with open('./twitter-50mb.json', 'r', encoding='utf-8') as twit_file:
    ...
    while (twit := twit_file.readline()) != '{}]}\n':

        # match count with rank to split work
        count += 1
        if count % size != rank:
            continue
...
if rank == 0:
    ...
    # avg_happy_hour_dict = {k: happy_hour_dict[k] / active_hour_dict[k] for k in happy_hour_dict}
    # avg_happy_day_dict = {k: happy_day_dict[k] / active_day_dict[k] for k in happy_day_dict}
    ...
    print(f"Happiest Hour: {happiest_hour}, Sentiment Score: {happy_hour_dict[happiest_hour]:.4f}")
    print(f"Happiest Day: {happiest_day}, Sentiment Score: {happy_day_dict[happiest_day]:.4f}")
    print(f"Most Active Hour: {most_active_hour}, Posts: {active_hour_dict[most_active_hour]}")
    print(f"Most Active Day: {most_active_day}, Posts: {active_day_dict[most_active_day]}")
